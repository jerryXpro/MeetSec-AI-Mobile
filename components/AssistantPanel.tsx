...
const AssistantPanel: React.FC = () => {
    const { messages, meetingTitle, temporaryFiles, addTemporaryFile, removeTemporaryFile } = useLive();
    ...
    const safeTemporaryFiles = temporaryFiles || [];
                        ...
                        <div className="flex-1 overflow-y-auto p-4 custom-scrollbar space-y-4">
                            ...
                            {!hasContent && !analysisResult ? (
                                <div className="flex flex-col mt-4">
                                    {/* Upload Box replaces Empty State */}
                                    <div className="p-5 rounded-2xl bg-zinc-900/80 border border-zinc-800 shadow-xl w-full max-w-lg mx-auto">
                                        <div className="flex items-center justify-between mb-4">
                                            <h3 className="text-sm font-bold text-zinc-300 flex items-center gap-2">
                                                <svg className="w-5 h-5 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 19a2 2 0 01-2-2V7a2 2 0 012-2h4l2 2h4a2 2 0 012 2v1M5 19h14a2 2 0 002-2v-5a2 2 0 00-2-2H9a2 2 0 00-2 2v5a2 2 0 01-2 2z" /></svg>
                                                補充參考資料
                                            </h3>
                                            <span className="text-[0.7em] text-zinc-500 bg-zinc-800/50 px-2 py-0.5 rounded-full">{safeTemporaryFiles.length} / 20</span>
                                        </div>
                                        <div className="space-y-3">
                                            <div className="max-h-48 overflow-y-auto custom-scrollbar pr-1 space-y-1.5 min-h-[80px]">
                                                {safeTemporaryFiles.map(file => (
                                                    <div key={file.id} className="group flex items-center justify-between bg-zinc-800/80 hover:bg-zinc-700/80 border border-zinc-700/50 hover:border-zinc-500 rounded-xl px-3 py-2 transition-all text-left">
                                                        <div className="flex items-center gap-3 overflow-hidden">
                                                            ...
                                                            <span className="text-[0.85em] font-medium text-zinc-300 truncate" title={file.name}>{file.name}</span>
                                                        </div>
                                                        <button onClick={() => removeTemporaryFile(file.id)} className="text-zinc-500 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all p-1.5 hover:bg-red-500/10 rounded-lg" title="移除">
                                                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                                                        </button>
                                                    </div>
                                                ))}
                                                {safeTemporaryFiles.length === 0 && (
                                                    <div className="h-full min-h-[120px] flex flex-col items-center justify-center text-zinc-500 border-2 border-dashed border-zinc-700/50 hover:border-blue-500/30 hover:bg-blue-500/5 transition-all rounded-xl relative cursor-pointer" onClick={() => contextFileInputRef.current?.click()}>
                                                        <svg className="w-8 h-8 opacity-50 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>
                                                        <span className="text-[0.85em] font-medium">點擊上傳欲分析的檔案</span>
                                                        <span className="text-[0.7em] opacity-70 mt-1">支援 PDF, Word, Excel, TXT 等</span>
                                                    </div>
                                                )}
                                            </div>
                                            ...
                                            <div className="grid grid-cols-2 gap-3 pt-2 border-t border-zinc-800/50 mt-4">
                                                ...
                                                <button 
                                                    disabled={safeTemporaryFiles.length === 0 || isAnalyzing}
                                                    onClick={() => handleAnalyze()}
                                                    className="py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white rounded-xl text-[0.85em] font-bold transition-all shadow-lg shadow-blue-900/30 flex items-center justify-center gap-2 border border-blue-500/50 disabled:opacity-50 disabled:grayscale"
                                                >
                                                    {isAnalyzing ? (
                                                        <svg className="animate-spin h-4 w-4" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                                                    ) : (
                                    ...
                                    <p className="text-center text-[0.85em] text-zinc-500 mt-6 select-none opacity-50">
                                        或是開始會議並錄音，AI 會記錄內容...
                                    </p>
                                </div>
                            ) : (
