...
const TranscriptLine: React.FC<TranscriptLineProps> = ({
    ...
    return (
        <div className={`group/line flex items-baseline hover:bg-white/5 rounded px-1 -mx-1 py-0.5 transition-colors leading-relaxed ${contentTextSize} ${isPlaying ? 'bg-blue-500/10' : ''}`}>
            ...
            {speakerNameRaw && (
                isRenamingSpeaker ? (
                    <div className="flex items-center gap-1 shrink-0 mr-2 bg-zinc-900 border border-amber-500/50 rounded px-1 py-0">
                        <input
                            type="text"
                            ...
                            onKeyDown={(e) => {
                                if (e.key === 'Enter') {
                                    setIsRenamingSpeaker(false);
                                    // Default to Single (Update Line Text directly)
                                    const newName = renameSpeakerValue.trim();
                                    if (newName && newName !== speakerNameRaw) {
                                        const newLine = rawLine.replace(speakerNameRaw, newName);
                            ...
                            autoFocus
                            className="font-bold text-amber-500 bg-transparent border-none text-sm outline-none w-20"
                        />
                        <button
                            onClick={(e) => {
                                e.stopPropagation();
                            ...
                            className="px-1 py-0 text-[10px] bg-zinc-800 text-zinc-300 hover:text-white rounded border border-zinc-700"
                            title="僅修改此行 (Single)"
                        >
                            單
                        </button>
...
const MessageBubble: React.FC<{ msg: Message }> = ({ msg }) => {
    const { updateMessage, renameSpeaker, updateMessageSpeaker } = useLive();
    ...
    return (
        <div className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div
                className={`max-w-[95%] lg:max-w-[85%] rounded-2xl px-4 py-3 shadow-sm transition-all duration-300 ${msg.role === 'user'
                ...
                {msg.role !== 'system' && (
                    <div className="flex flex-col gap-2 mb-2">
                        ...
                        <div className="flex items-center gap-2 group/header flex-wrap">
                            {isEditingHeaderName ? (
                                <div className="flex items-center gap-1 bg-zinc-900 border border-zinc-600 rounded px-1 py-0.5">
                                    <input
                                        type="text"
                                        ...
                                        onKeyDown={(e) => {
                                            if (e.key === 'Enter') {
                                                // Default to Single on Enter if no button clicked
                                                setIsEditingHeaderName(false);
                                                const newName = headerNameInput.trim();
                                                if (newName && newName !== (msg.speaker || '')) {
                                                    updateMessageSpeaker(msg.id, newName);
                                                }
                                            } else if (e.key === 'Escape') {
                                                setIsEditingHeaderName(false);
                                            }
                                    ...
                                    />
                                    <div className="flex bg-zinc-800 rounded border border-zinc-700 overflow-hidden shrink-0">
                                        <button
                                            onClick={() => {
                                                setIsEditingHeaderName(false);
                                                const newName = headerNameInput.trim();
                                                if (newName) updateMessageSpeaker(msg.id, newName);
                                            }}
                                            className="px-1.5 py-0.5 text-[10px] text-zinc-300 hover:bg-zinc-700 hover:text-white border-r border-zinc-700 transition-colors"
                                            title="僅修改此則 (Single Edit)"
                                        >
                                            單筆
                                        </button>
                                        <button
                                            onClick={() => {
                                                ...
                                                const currentName = msg.speaker || (msg.role === 'user' ? 'User / Audio' : 'AI');
                                                const newName = headerNameInput.trim();
                                                if (newName && newName !== currentName) renameSpeaker(currentName, newName);
                                            }}
                                            className="px-1.5 py-0.5 text-[10px] text-zinc-300 hover:bg-zinc-700 hover:text-white transition-colors"
                                            title="全部取代 (Batch Rename)"
                                        >
                                            全部
                                        </button>
                                    </div>
                                ...
                                <div
                                    ...
                                    className={`flex items-center gap-2 cursor-pointer hover:bg-white/5 rounded px-1 -ml-1 transition-colors ${msg.role === 'user' ? 'text-blue-400' : 'text-purple-400'
                                        }`}
                                    title="修改發言者"
                                >
                            ...
                            {msg.audioUrl && !msg.isPartial && !isRichTranscript && (
                                <button
                                    ...
                                    className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium transition-all border ${isPlaying
...
const Transcript: React.FC = () => {
    ...
    return (
        <div
            ...
            className="flex-1 overflow-y-auto p-4 pb-56 md:pb-48 space-y-4 relative custom-scrollbar h-full"
        >
            {messages.length === 0 && (
